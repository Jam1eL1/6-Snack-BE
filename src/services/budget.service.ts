...
const assertCompanyAccess = (
  ...
  authenticatedCompanyId: MonthlyBudget["companyId"],
) => {
  if (companyId !== authenticatedCompanyId) {
    throw new ForbiddenError("You do not have permission to access this company's budget.");
  }
};
...
const getMonthlyBudget = async (
  ...
  assertCompanyAccess(companyId, authenticatedCompanyId);
